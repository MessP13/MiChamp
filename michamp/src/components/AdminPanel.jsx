...
export default function AdminPanel({ data, setData, simDate, onClose, onUpdateMatch }) {
  ...
  const [actionPending, setActionPending] = useState(null); // NOVO: controle do modal
  ...
  // CORREÇÃO: flatMap aninhado com parênteses corretos
  ...
  // ATUALIZADO: captura actionRequired do resultado
  const doSuggestSolos = (cId) => {
    ...
    // Se houver ação requerida, mostra modal
  ...
  // NOVO: handler para decisões do modal
  const handleSoloDecision = (champId, actionType) => {
    ...
    if (actionType === 'merge' && solos.length >= 2) {
      const newTeam = {
        id: Date.now(), tipo:"time", nome: `Time Solo (Agrupado)`, riot_tag:"#TS",
        estado:"aprovado", capitao:null,
    ...
    setSoloGroups(prev => ({
      ...prev,
      [champId]: { ...prev[champId], leftover: [], actionRequired: false, suggestedAction: null }
    }));
  };
  ...
  return (
    <div>
      ...
      {/* NOVO: Modal de decisão para solos */}
      {actionPending && (
        <div style={{
          ...
          <div style={{
            ...
            {actionPending.leftover?.length > 0 && (
              <div style={{ background:"var(--el)", borderRadius:8, padding:"10px", marginBottom:16 }}>
                <p style={{ fontSize:11, color:"var(--muted)", marginBottom:8 }}>
                  Jogadores: {actionPending.leftover.map(s => s.nick).join(", ")}
                </p>
              </div>
            )}
              ...
              {actionPending.action.options.map(opt => (
                <button key={opt.id} 
                  ...
                  style={{
                    ...
                    border:"0.5px solid var(--bdm)", background:"var(--el)",
          ...
          {data.filter(c => c.participantes.some(p => p.tipo==="solo" && p.estado==="aprovado")).map(c => {
            ...
            return (
              <div key={c.id} style={{ ...elStyle, marginBottom:12 }}>
                ...
                {sug?.stats && (
                  <div style={{ display:"flex", gap:12, marginBottom:12, padding:"8px 12px", background:"var(--card)", borderRadius:8, fontSize:11 }}>
                    <span style={{ color:"var(--muted)" }}>Total: <strong>{sug.stats.total}</strong></span>
                    <span style={{ color:"var(--ok)" }}>Agrupados: <strong>{sug.stats.grouped}</strong></span>
                    <span style={{ color: sug.stats.leftover > 0 ? "var(--warn)" : "var(--muted)" }}>Restantes: <strong>{sug.stats.leftover}</strong></span>
                  </div>
                )}
                ...
                {sug && <>
                  ...
                  {sug.leftover?.length > 0 && !actionPending && (
                    <p style={{ fontSize:11, color:"var(--warn)", marginTop:4 }}>
                      ⚠️ {sug.leftover.length} jogador(es) sem time. Clique em "Sugerir times" novamente.
                    </p>
                  )}
                </>}
              </div>
